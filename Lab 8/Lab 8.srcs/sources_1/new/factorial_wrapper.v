...
module factorial_wrapper(
    ...
    wire we1, we2, go_out, go_in, we2_and_write_data_0, done, error;
    wire [1:0] rd_sel;
    wire [3:0] n;
    wire [31:0] prod;
    ...
    factorial_CU_DP_wrap fact(.clk(clk), .GO(go_in), .rst(rst), .n({28'b0, n_reg}), .DONE(done), .product(prod), .error(error));
